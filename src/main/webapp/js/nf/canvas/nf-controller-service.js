...
(function (root, factory) {
    ...
    var disableHandler = function (serviceTable) {
        ...
        if (hasUnauthorizedReferencingComponent(controllerService.referencingComponents)) {
            ...
            nfDialog.showOkDialog({
                headerText: '控制器服务',
                dialogContent: '由于未经授权引用组件而无法禁用.'
            });
            return;
        }

        $('#disable-progress-label').text('禁用' + controllerService.name + '的步骤');
        ...
        $.Deferred(function (deferred) {
            ...
            if (canceled === true && $('#nf-ok-dialog').not(':visible')) {
                nfDialog.showOkDialog({
                    headerText: '控制器服务',
                    dialogContent: '禁用的请求被取消了. 这个请求的部分可能已经完成. 请校验这个服务和所有引用组件的状态.'
                });
            }
        });
    };
    ...
    var enableHandler = function (serviceTable) {
        ...
        if (scope === config.serviceAndReferencingComponents && hasUnauthorizedReferencingComponent(controllerService.referencingComponents)) {
            ...
            nfDialog.showOkDialog({
                headerText: '控制器服务',
                dialogContent: '由于未经授权引用组件而无法启用.'
            });
            return;
        }

        $('#enable-progress-label').text('开启' + controllerService.name + '的步骤');
        ...
        $.Deferred(function (deferred) {
            ...
            if (canceled === true && $('#nf-ok-dialog').not(':visible')) {
                nfDialog.showOkDialog({
                    headerText: '控制器服务',
                    dialogContent: '启用的请求被取消了. 这个请求的部分可能已经完成. 请校验这个服务和所有引用组件的状态.'
                });
            }
        });
    };
