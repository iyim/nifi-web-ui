...
(function (root, factory) {
        ...
        $.widget('nf.userSearchAutocomplete', $.ui.autocomplete, {
            ...
            _renderMenu: function (ul, items) {
                ...
                if (ul.children().length === 0) {
                    ul.append('<li class="unset search-no-matches">没有用户匹配搜索</li>');
                }
            },
    ...
    var initPolicyTable = function () {
                        ...
                        if (option.value === 'restricted-components') {
                            if (!initializedComponentRestrictions) {
                                ...
                                var options = [{
                                    ...
                                    value: '',
                                    description: '允许用户不受限制地创建/修改所有受限制的组件.'
                                }];
                                ...
                                requiredPermissions.each(function (label, id) {
                                    if (id !== option.value) {
                                        options.push({
                                            ...
                                            value: id,
                                            description: "允许用户创建/修改受限组件，需要 '" + nfCommon.escapeHtml(label) + "'"
                                        });
                                    }
                                });
        ...
        $('#component-policy-target').combo({
            options: [{
                ...
                description: '允许用户修改组件配置信息'
            }, {
                text: 'view provenance',
                value: 'read-provenance',
                description: '允许用户查看此组件生成的起源事件'
            }, {
                text: 'view the data',
                value: 'read-data',
                description: '允许用户在出站连接和源项事件中查看流组件队列中此组件的元数据和内容'
            }, {
                text: 'modify the data',
                value: 'write-data',
                description: '允许用户在出站连接中清空流文件队列，并通过源项事件提交重播'
            }, {
                text: 'receive data via site-to-site',
                value: 'write-receive-data',
                description: '允许此端口从这些NiFi实例接收数据',
                disabled: true
            }, {
                text: 'send data via site-to-site',
                value: 'write-send-data',
                description: '允许此端口将数据发送到这些NiFi实例',
                disabled: true
            }, {
                text: 'view the policies',
                value: 'read-policies',
                description: '允许用户查看可以查看/修改此组件的用户列表'
            }, {
                text: 'modify the policies',
                value: 'write-policies',
                description: '允许用户修改可以查看/修改此组件的用户列表'
            }],
            select: function (option) {
                if (initialized) {
    ...
    var promptToRemoveUserFromPolicy = function (item) {
        nfDialog.showYesNoDialog({
            headerText: '更新策略',
            dialogContent: '从这条策略中移除 \'' + nfCommon.escapeHtml(item.component.identity) + '\' ?',
            yesHandler: function () {
            ...
            policyDeferred = $.Deferred(function (deferred) {
                $.ajax({
                    ...
                    if (xhr.status === 404) {
                        ...
                        // show an appropriate message
                        $('#policy-message').text('未授权访问指定资源的策略.');
                ...
                $.ajax({
                    ...
                    if (xhr.status === 404) {
                        ...
                        if (resourceAndAction.resource === '/restricted-components') {
                            $('#policy-message').text('没有用户有"不受限制"的权限');
                        } else {
                            $('#policy-message').text('没有具有特定限制权限的用户.');
                        }
